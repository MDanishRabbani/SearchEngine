...
for (i = 0; i < totalqterm; i++) {
    ...
    if (pos < 0) {
        ...
        for (t = 0; t < fw; t++) {
            docno = ilbuf[t * 2] + fileinfo[0].docno; // Adjust docno by adding starting document number

            // Ensure docno is within a valid range
            if (docno >= 0 && docno < N) {
                freqwd = ilbuf[t * 2 + 1];

                if (N > 0 && fw > 0 && freqwd > 0) {
                    if (N / fw > 0 && freqwd > 0) {
                        s_q_d = log1p(N / fw) * log1p(freqwd);
                        if (!isnan(s_q_d) && isfinite(s_q_d)) {
      ...
      for (i = 0; i < HEAPSIZE; i++) {
        if (heap[i].docno >= 0 && heap[i].docno < N) {
...
/* sort is a function to sort query terms by their frequency value in ascending
   order, rare term should examine first in order to heuristically limit the
   accumulator */
